...
3. Prefer the bundled wrapper:
   ```bash
   SKILL_DIR="${AGENTS_SKILLS_DIR:-$HOME/.agents/skills}/unlimited-ocr"
   INPUT_PATH="/path/to/input.pdf"
   python "$SKILL_DIR/scripts/run_unlimited_ocr.py" "$INPUT_PATH"
   ```
...
SKILL_DIR="${AGENTS_SKILLS_DIR:-$HOME/.agents/skills}/unlimited-ocr"
INPUT_PATH="/path/to/input.pdf"
python "$SKILL_DIR/scripts/run_unlimited_ocr.py" "$INPUT_PATH" --monitor
```
...
The source PDF, image, or image directory, raw page Markdown outputs, and combined or polished OCR Markdown are user-owned inputs and outputs. Uninstalling this skill must not delete any of them.
...
RUN_DIR="$HOME/Unlimited-OCR/outputs/manual"
...
For image directories, set `IMAGE_DIR`, replace `--pdf "$INPUT_PDF"` with `--image_dir "$IMAGE_DIR"`, and use `--image_mode gundam`.

After a direct run, build the readable draft:

```bash
SKILL_DIR="${AGENTS_SKILLS_DIR:-$HOME/.agents/skills}/unlimited-ocr"
PAGES_DIR="$HOME/Unlimited-OCR/outputs/manual/pages"
OUTPUT_MD="/path/to/input.md"
python "$SKILL_DIR/scripts/build_readable_markdown.py" "$PAGES_DIR" \
  --output "$OUTPUT_MD"
```
