...
3. Create a temporary working directory only if needed for copied intermediates, using a timestamped name such as `tmp/restoration-20260711-153000` inside the current workspace.
...
7. Confirm the `$imagegen` output is accessible as a file that can be copied or moved. If no file appears under `$CODEX_HOME/generated_images/...`, check whether the built-in tool stored the result inline in Codex session JSONL. Set `OUTPUT_DIR` and `ORIGINAL_STEM` to the concrete values selected earlier, then run:
   ```bash
   SKILL_DIR="${AGENTS_SKILLS_DIR:-$HOME/.agents/skills}/restoration"
   OUTPUT_DIR="${OUTPUT_DIR:?set OUTPUT_DIR to the selected output directory}"
   ORIGINAL_STEM="${ORIGINAL_STEM:?set ORIGINAL_STEM to the original filename without its extension}"
   python "$SKILL_DIR/scripts/extract_latest_imagegen_result.py" \
     --out-dir "$OUTPUT_DIR" \
     --stem "${ORIGINAL_STEM}-imagegen-enhanced"
   ```
...
9. Save the final selected `$imagegen` output to the selected output directory:
   ...
   - The Windows Pictures path should normally resolve under `/mnt/c/Users/` from WSL and correspond to the user's `Pictures` directory on Windows.
...
10. Use a descriptive, non-overwriting filename such as `${ORIGINAL_STEM}-imagegen-enhanced.png`; if the file exists, append `-v2`, `-v3`, and so on.
...
Use `$SKILL_DIR/scripts/extract_latest_imagegen_result.py` only to extract the built-in `$imagegen` PNG result. Do not use it to transform, sharpen, redraw, or otherwise alter the image.

## Data Ownership and Uninstall

The original input image, the final restored image, Codex session JSONL files, and files in Codex's `generated_images` directories are user or shared data, not installed Restoration skill state. Uninstalling or removing this skill must leave all of them intact. Cleanup is limited to temporary intermediates created for the current restoration and must never delete these user or shared data.
