...
- Output filenames must be based on the user's original audio/video filename, not the intermediate mono Opus filename.
...
- Fun-ASR accepts Opus for non-realtime transcription at arbitrary sample rates. The helper uses mono 16 kHz Opus at 32 kbit/s to reduce temporary-upload size without disabling diarization.
...
```bash
ffmpeg -y -i input.mp4 -vn -ac 1 -ar 16000 -c:a libopus -b:a 32k -application voip output.opus
```
