...
```bash
SKILL_DIR="${AGENTS_SKILLS_DIR:-$HOME/.agents/skills}/asr"
...
2. For local files, run ffmpeg synchronously.
   ...
   - Normalize all local media to compact mono 16 kHz Opus for diarization.
...
4. Submit the async Fun-ASR task and poll it.
   ...
   - Immediately save the submitted task ID in a hidden `.<stem>.asr-task.json` checkpoint beside the eventual transcript.
   - If polling is interrupted or times out, rerun the same command to resume the matching cloud task without converting, uploading, or submitting again.
   - Use `--restart` only when the saved task must be discarded and a new cloud task submitted.
5. Save results.
   - Save only a plain text transcript.
   - Name the output from the original user input file stem, such as `meeting.mp4` -> `meeting.txt`.
   - Do not name the output after intermediate converted files such as `meeting.mono16k.opus`.
   - Treat transcript `.txt` files as user-owned output. Uninstalling or removing this skill must not delete them.
6. Cleanup.
   - Use an independent temporary directory for each input.
   - Remove temporary conversion files for each finished input.
   - Do not keep downloaded JSON task/result files. Remove the local resume checkpoint after success or terminal task failure; retain it only while a submitted task may still be resumed.
...
python "$SKILL_DIR/scripts/transcribe.py" input.mov --output-dir out

# Local video or audio file, output next to input by default
python "$SKILL_DIR/scripts/transcribe.py" /path/to/input.mov
...
python "$SKILL_DIR/scripts/transcribe.py" input.wav \
  --output-dir out --language-hints zh,en --speaker-count 2

# Discard a saved pending-task checkpoint and submit a fresh task
python "$SKILL_DIR/scripts/transcribe.py" input.wav --restart
```
...
Async recognition can run longer than the source media. A runner-level deadline must be longer than the script's `--timeout` (six hours by default), or disabled. If the runner is interrupted, rerun the same command and let the checkpoint resume the existing task instead of creating a duplicate billable task.
