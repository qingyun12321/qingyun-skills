...
UPLOAD_CONNECT_TIMEOUT_SECONDS = 30
UPLOAD_LOW_SPEED_LIMIT_BYTES = 1024
UPLOAD_LOW_SPEED_TIME_SECONDS = 120
...
def input_fingerprint(media: str) -> str:
    if is_url(media):
        identity: dict[str, Any] = {"url": media}
    else:
...
def checkpoint_path(output_dir: Path, stem: str) -> Path:
    return output_dir / f".{stem}.asr-task.json"
...
def load_checkpoint(
    ...
    *,
    ...
    if (
        ...
        or state.get("api_host") != api_host
...
def prepare_local_audio(input_path: Path, temp_dir: Path) -> Path:
    ...
    output = temp_dir / f"{input_path.stem}.mono16k.opus"
    cmd = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel",
...
def upload_temporary_file(api_key: str, model_name: str, path: Path) -> str:
    ...
    cmd = [
        ...
        "-fsS",
        "--connect-timeout",
        str(UPLOAD_CONNECT_TIMEOUT_SECONDS),
        "--speed-limit",
        str(UPLOAD_LOW_SPEED_LIMIT_BYTES),
        "--speed-time",
        str(UPLOAD_LOW_SPEED_TIME_SECONDS),
        "--max-time",
    ...
    try:
        subprocess.run(cmd, check=True, timeout=UPLOAD_TIMEOUT_SECONDS + 5)
    except subprocess.TimeoutExpired as exc:
        raise AsrError(
            f"temporary upload timed out after {UPLOAD_TIMEOUT_SECONDS} seconds"
        ) from exc
    except subprocess.CalledProcessError as exc:
        raise AsrError(f"temporary upload failed with curl exit code {exc.returncode}") from exc
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--restart",
        action="store_true",
        help="Ignore a matching saved task checkpoint and submit a new transcription task",
    )
...
def transcribe_one(args: argparse.Namespace, media: str) -> Path:
    with tempfile.TemporaryDirectory(prefix="asr-") as temp:
        ...
        fingerprint = input_fingerprint(media)
        checkpoint = checkpoint_path(output_dir, stem)
        if args.restart:
        ...
        if task_id:
            print(f"{original_name or media}: resuming task {task_id}", file=sys.stderr)
        else:
            if is_url(media):
                file_url = media
            else:
        ...
        if query.get("output", {}).get("task_status") != "SUCCEEDED":
            checkpoint.unlink(missing_ok=True)
